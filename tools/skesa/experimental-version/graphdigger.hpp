...
************************/
    ...
    struct SContig {
        ...
        SContig(const string& contig, CDBGraph& graph) : 
            m_next_left(0), m_next_right(0), m_left_link(nullptr), m_left_shift(0), m_right_link(nullptr), m_right_shift(0), 
            m_left_extend(0), m_right_extend(0), m_kmer_len(graph.KmerLen()), m_is_taken(0) {

            m_seq.resize(1); // one empty list
            m_seq.front().emplace_front(contig.begin(), contig.end());

            GenerateKmers(graph);
            for(auto& lst : m_kmers) {
                for(auto& node : lst) {
                if(node)
                    graph.SetVisited(node);   // may be 0                   
                }
            }
        }
...
//          graph - de Bruijn graph
            ...
            m_seq.resize(1);         // one empty list
            m_seq.front().resize(1); // one empty deque
            auto& seq = m_seq.front().front();            
            for(const auto& base : to_left)
                seq.push_front(Complement(base.m_nt));
            string ikmer = graph.GetNodeSeq(initial_node);
            seq.insert(seq.end(), ikmer.begin(), ikmer.end());
            for(const auto& base : to_right)
                seq.push_back(base.m_nt);
            GenerateKmers(graph);

            m_left_extend = m_right_extend = Len();
        }
        SContig(SContig* link, int shift, CDBGraph::Node takeoff_node, const TBases& extension, CDBGraph::Node rnode, const CDBGraph& graph) :
            ...
            seq.insert(seq.end(), kmer.begin()+1, kmer.end());
        ...
        void GenerateKmers(const CDBGraph& graph) {
            //TODO create kmers for all variants

            TVariation first_variant;
            for(auto& lst : m_seq)
            ...
            for(CReadHolder::kmer_iterator itk = rh.kbegin(m_kmer_len); itk != rh.kend(); ++itk)  // gives kmers in reverse order!
                m_kmers.emplace_front(1, graph.GetNode(*itk));
        }

        CDBGraph::Node FrontKmer() const { return m_kmers.front().front(); } // front must be unambiguous
        CDBGraph::Node BackKmer() const { return m_kmers.back().front(); }   // back must be unambiguous
        ...
        void AddToRight(const SContig& other) { 
            if(other.m_right_extend < (int)other.Len()) {
                m_right_extend = other.m_right_extend;
            } else {
                m_right_extend += other.m_right_extend-m_kmer_len+1;
                if(m_left_extend == (int)Len())
                    m_left_extend = m_right_extend;
            }

            auto& last_chunk = m_seq.back().front();
            auto& first_other_chunk = other.m_seq.front().front();
            last_chunk.insert(last_chunk.end(), first_other_chunk.begin()+m_kmer_len-1, first_other_chunk.end());  // first/last chunks must have at least kmer_len-1 unambiguous sequence
            m_seq.insert(m_seq.end(), other.m_seq.begin()+1, other.m_seq.end());                                   // insert remaining chunks
        ...
        void AddToLeft(const SContig& other) {
            if(other.m_left_extend < (int)other.Len()) {
                m_left_extend = other.m_left_extend;
            } else {
                m_left_extend += other.m_left_extend-m_kmer_len+1; 
                if(m_right_extend == (int)Len())
                    m_right_extend = m_left_extend;
            }

            auto& first_chunk = m_seq.front().front();
            auto& last_other_chunk = other.m_seq.back().front();
            first_chunk.insert(first_chunk.begin(),last_other_chunk .begin(), last_other_chunk.end()-m_kmer_len+1);  // first/last chunks must have at least kmer_len-1 unambiguous sequence
            m_seq.insert(m_seq.begin(), other.m_seq.begin(), other.m_seq.end()-1);                                   // insert remaining chunks
        ...
        pair<size_t, TNodeList::const_iterator> MinKmerPosition() const {  // could be in the middle of ambiguous seq
            size_t mkp = 0;
            auto itmk =  m_kmers[mkp].begin();
            for(size_t i = 0; i < m_kmers.size(); ++i) {
                for(auto it =  m_kmers[i].begin(); it !=  m_kmers[i].end(); ++it) {
                    if(*it != 0 && (*itmk == 0 || *it < *itmk)) {
                        mkp = i;
                        itmk = it;
                    }
                }
            }
            return make_pair(mkp, itmk);
        }

        // stabilize contig orientation using minimal kmer in the contig
        void SelectMinDirection() {
            CDBGraph::Node minkmer = *MinKmerPosition().second;
        ...
        void RotateCircularToMinKmer() { // assumes that the next extension of sequence would give the first kmer (m_next_right == m_kmers.front())
            ...
            size_t first_base = rslt.first;
            size_t first_chunk = 0;
            if((*rslt.second)%2) { // cut after the minimal kmer so rotate brings it to the beginning
                first_base = (first_base+m_kmer_len)%m_kmers.size();
                while(m_seq[first_chunk].front().size() <= first_base)
                    first_base -=  m_seq[first_chunk++].front().size();
                if(distance(m_seq[first_chunk].begin(), m_seq[first_chunk].end()) > 1) { // ambiguous interval - we don't want to cut it
                    first_base = 0;
                    if(++first_chunk == m_seq.size())
                        first_chunk = 0;
                }
            } else {
                while(m_seq[first_chunk].front().size() <= first_base)
            ...
            rotate(m_seq.begin(), m_seq.begin()+first_chunk, m_seq.end());
            if(first_base > 0) {
                m_seq.resize(m_seq.size()+1);
                m_seq.back().emplace_front(m_seq.front().front().begin(), m_seq.front().front().begin()+first_base);
        ...
        static TContigList ConnectFragments(vector<TContigList>& fragments, const CDBGraph& graph) {
                ...
                for(auto iloop = ns.begin(); iloop != ns.end(); ) {
                    ...
                    if(contig.m_next_left) {
                        ...
                        if(!rslt.second) {
                            ...
                            if(contig.m_kmers.empty() && contig.m_left_link && !other->m_left_link && contig.m_next_right == (other->m_kmers.empty() ? other->m_next_right:other->FrontKmer())) {
                                other->AddToLeft(contig); // add left link to other
                                connected.pop_front();
                                continue;
                            } else if(other->m_kmers.empty() && other->m_left_link && !contig.m_left_link && other->m_next_right == (contig.m_kmers.empty() ? contig.m_next_right:contig.FrontKmer())) {
                    ...
                    if(contig.m_next_right) {
                        ...
                        if(!rslt.second) {
                            TContigList::iterator other = rslt.first->second;
                            if(contig.m_kmers.empty() && contig.m_right_link && !other->m_right_link && contig.m_next_left == (other->m_kmers.empty() ? other->m_next_left:other->BackKmer())) {
                                other->AddToRight(contig); // add right link to other
                                denied_left_nodes.erase(contig.m_next_left);
                                connected.pop_front();
                            } else if(other->m_kmers.empty() && other->m_right_link &&  !contig.m_right_link && other->m_next_left == (contig.m_kmers.empty() ? contig.m_next_left:contig.BackKmer())) {
            ...
            for(SContig& contig : connected) {
                ...
                while(keep_doing) {
                    keep_doing = false;
                    if(contig.m_next_right) {
                        CDBGraph::Node rnode = contig.BackKmer();
                    ...
                    if(contig.m_next_left) {
                        CDBGraph::Node lnode = contig.FrontKmer();
                ...
                if(contig.m_next_right == contig.FrontKmer() && (int)contig.m_kmers.size() >= graph.KmerLen())  // circular and not very short  
                        ...
                        if((plus && (rslt = right_connections.find(fragment)) != right_connections.end()) ||
                           (!plus && (rslt = left_connections.find(fragment)) != left_connections.end())) {
                            ...
                            if(connector->m_right_link == fragment) { // either reversed or circular            
                                ...
                                if(rshift < (int)contig.m_kmers.size() && CDBGraph::ReverseComplement(connector->m_next_right) == (contig.m_kmers.end()-rshift-1)->front())
                                    connector->ReverseComplement();
                            }
                            int lshift = connector->m_left_shift;
                            contig.ClipRight(lshift > 0 ? lshift-1 : -(lshift+1));
                            if(connector->m_left_link != fragment || contig.BackKmer() != connector->m_next_left)
                            ...
                            if(fragment->FrontKmer() != connector->m_next_right)
        ...
        TSequence m_seq;                 // sequence
        deque<TNodeList> m_kmers;        // kmers
    ...
    private:
        ...
        void ExtendContigsJob(TContigList& scontigs, TContigList& extensions, int scan_window) {
            ...
            for(auto& contig : scontigs) {
                ...
                scan_window = 0;                             //!!!!!!!!!!!!!!!!!!!!!!REMOVE
                int sw = min(scan_window,(len-kmer_len)/2);  // TODO scan window in ambiguous area 

                for(int shift = 0; shift <= sw; ++shift) {
                    CDBGraph::Node takeoff_node = contig.m_kmers[klen-shift-1].front();
                    if(takeoff_node && GoodNode(takeoff_node)) {         // valid kmer 
                        ...
                        if(!extension.first.empty() || extension.second) { // extension could be empty - starting kmer + landing kmer
                            if(shift == 0 || !extension.second || extension.second != contig.m_kmers[klen-shift].front()) {
                ...
                for(int shift = 0; shift <= sw; ++shift) {
                    CDBGraph::Node takeoff_node = CDBGraph::ReverseComplement(contig.m_kmers[shift].front());
                    if(takeoff_node && GoodNode(takeoff_node)) {         // valid kmer     
                        pair<TBases, CDBGraph::Node> extension = ExtendToRight(takeoff_node);
                        if(!extension.first.empty() || extension.second) { // extension could be empty - starting kmer + landing kmer   
                            if(shift == 0 || !extension.second  || extension.second != CDBGraph::ReverseComplement(contig.m_kmers[shift-1].front())) {
                                SContig sc(&contig, -(shift+1), takeoff_node, extension.first, extension.second, Graph());
