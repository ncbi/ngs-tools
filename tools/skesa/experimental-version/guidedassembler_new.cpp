    ...
    void CleanContigJob(TContigs::iterator contigp, TContigs& rslt) {
        ...
        while(true) { // loop through all ambiguous symbols combinations
            ...
            auto ScanLeftRight = [&](int insert, char skip_type) {
                ...
                size_t hit_num = 0;
                int max_extend = 0;
                deque<pair<int,int>> clip_counts(insert);

                double accum_read_len = 0;
                int accum_read_num = 0;
                int buff = (skip_type == 'A' ? max_read_len/2 : max_read_len);
                deque<pair<int,double>> prev_reads(buff);
                for(int from = 0; from < contig_len; ++from) {
                    clip_counts.pop_front();
                    clip_counts.emplace_back(0,0);
                    --max_extend;

                    accum_read_num -= prev_reads.front().first;
                    accum_read_len -= prev_reads.front().second;
                    ...
                    for( ; hit_num < hits_for_variant.size() && get<0>(hits_for_variant[hit_num]) == from; ++hit_num) {
                        ...
                        if(get<2>(hits_for_variant[hit_num]) > extend)
                            ++clip_counts[extend].second;                       

                        ++accum_read_num;
                        accum_read_len += get<2>(hits_for_variant[hit_num]);
                        ++prev_reads.back().first;
                        prev_reads.back().second += hit_insert;

                        double abundance_to = labundance[from+extend];
                        if(abundance_to == 0)
                            abundance_to = abundance_from;
                        if(0.5*(abundance_from+abundance_to) > 2*m_average_count)
                            continue;
                        
                        if(get<2>(hits_for_variant[hit_num]) >= extend+critical_extend)
                            ++clip_counts[extend].first;                       
                    }
                    int to = from+max_extend;
                    if(to >= contig_len-1)
                        break;

                    double abundance_to = labundance[to];
                    double abundance_after = rabundance[to+1];
                    if(abundance_to == 0)
                        abundance_to = abundance_from;

                    if((abundance_after == 0 || abundance_after >= 10) &&  abundance_to >= 10 && 0.5*(abundance_from+abundance_to) <= 2*min(abundance_after,m_average_count) && max_extend >= fuzz) {
                        int count = accumulate(clip_counts.begin()+max_extend-fuzz, clip_counts.begin()+max_extend+1, 0, [](int t, const pair<int,int>& c) { return t+c.first; });
                        if(count >= critical_count) {
                            return make_pair(true, to);                                        
                        }
                    }

                    if(accum_read_num > 10 &&  max_read_len >= m_kmer_len && to > insert) {                              // TODO use gauss instead of delta func
                        ...
                        double margin = 15./density;                                                                     // read length distribution is delta func around rlen
                        if(max_extend < 0.9*(rlen-1)-margin)                                                                 
                            return make_pair(true, to);
                    } 

                    if(max_extend > 0 && clip_counts[max_extend].second > 5 && to >= insert-1) {
                        ...
                        if(successors.empty()) {
                            ...
                            if(fraction == 0 || pow(1.-fraction, clip_counts[max_extend].second) < 1e-4)
                                return make_pair(true, to);                        
                        }
                    }
                }
            ...
            auto ScanRightLeft = [&](int insert, char skip_type) {
                ...
                for(int from = contig_len-1; from >=0; --from) {
                    ...
                    for( ; hit_num < hits_for_variant.size() && get<0>(hits_for_variant[hit_num]) == from; ++hit_num) {
                        ...
                        double abundance_to = rabundance[from-extend];
                        if(abundance_to == 0)
                            abundance_to = abundance_from;
                        if(0.5*(abundance_from+abundance_to) > 2*m_average_count)
                            continue;
                    ...
                    double abundance_to = rabundance[to];
                    double abundance_after = labundance[to-1];
                    if(abundance_to == 0)
                        abundance_to = abundance_from;
                    ...
                    if(accum_read_num > 10 &&  max_read_len >= m_kmer_len && to < contig_len-insert) {                              // TODO use gauss instead of delta func
                        double expected_count = m_average_count;
                        if(skip_type == 'D')
                            expected_count /= 2;                                                                         // there are 2x less pairs than reads
                        double density = min(double(accum_read_num)/buff, expected_count/(max_read_len-m_kmer_len+1));   // number of reads/pairs per genome base (coverage/rlen)
                        double rlen = accum_read_len/accum_read_num;                                                     // local average read/pairs lengts
                        double margin = 15./density;                                                                     // read length distribution is delta func around rlen
                        if(max_extend < 0.9*(rlen-1)-margin)                                                                 // 0.9 compensates for simple model and statistical errors
                            return make_pair(true, to);
                    }

                    if(max_extend > 0 && clip_counts[max_extend].second > 5 && to <= contig_len-insert) {
            ...
            pair<bool, int> skip;
            {
               ...
               //               skip = Scan(max_read_len, true, skip_type);            
            ...
            if(!skip.first) {
                ...
                //                skip = Scan(max_read_len, false, skip_type);
            ...
            if(!skip.first) {
                ...
                //                skip = Scan(insert, true, skip_type);
            ...
            if(!skip.first) {
                ...
                //                skip = Scan(insert, false, skip_type);
