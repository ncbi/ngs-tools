...
struct Hash
{
    ...
    static void for_all_hashes_do(const std::string &s, int kmer_len, Lambda &&lambda) // todo: merge with the code below
    {
    ...
    template <class Lambda>
    static void for_all_hashes_do(const char *s, int len, int kmer_len, Lambda &&lambda)
    {
        if (len < kmer_len)
            return;

        auto hash = hash_of(s, kmer_len);
        for (int i=0; i <= len - kmer_len; i++, hash = hash_next(&s[i], hash, kmer_len))
            if (!lambda(hash))
                break;
    }
