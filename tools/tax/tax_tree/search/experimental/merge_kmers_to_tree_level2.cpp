...
#include <set>
#include <map>
#include "config_merge_kmers_to_tree_level2.h"
...
void merge_kmers_to_tree(TreeBuilder<SimilarityOf3Mers> &builder, const string &filename, int data_frame, int window_len, int between_windows_step)
{
    SeqLoader::for_every_clean_sequence_do(filename, [&](p_string line)
    {
		for (int window_from = data_frame; window_from <= line.len - window_len; window_from += between_windows_step)
            builder.insert(calculate_features_for_3mers(line.s + window_from, window_len, builder.similarity_of.tree_3mers));
    });
};
...
void merge_kmers_to_tree_list(const Config &config)
{
    ...
    TreeBuilder<SimilarityOf3Mers> builder(sim3);
    ...
    for (int file_number = 0; file_number < int(file_list.files.size()); file_number ++)
    {
        ...
        merge_kmers_to_tree(builder, file.filename, config.data_frame, config.window_len, config.between_windows_step);
...
int main(int argc, char const *argv[])
{
    try
    {
		cerr << "merge_kmers_to_tree_level2 version " << VERSION << endl;
