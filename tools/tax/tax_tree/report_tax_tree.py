...
def to_str(x):
#	x = float(x) # todo: isinstance(int, long)
	if isinstance(x, (int, long)): # x.is_integer():
		return str(x)
	return ("%.2f" % x) + '%'

def print_tree(roots, level = 0, min_count = 0):
	roots = [(line, roots[line].nodes, roots[line].weight) for line in roots]
	roots.sort(key = lambda(line, subtree, count) : - count)
	if not roots:
		return
	...
	for line, subtree, count in roots:
		...
		if level >= 2 and (count < top_count/3 or count < min_count):
			print "..."
			return

		print line, to_str(count)
		print_tree(subtree, level + 1, min_count)
